...
void neapu::HttpServer::HttpLog(bool _log) { m_log = _log; }
...
void neapu::HttpServer::StaticPath(const String &_reqPath,
  ...
  if (m_staticPath.m_filePath.Back() != '\\' &&
      m_staticPath.m_filePath.Back() != '/') {
#ifdef _WIN32
...
void neapu::HttpServer::OnRequestStaticFile(
  ...
  //排除相对路径，防止跨目录攻击
  if (!m_staticPath.m_enableRelativePath &&
      (_reqPath.Contain("../") || _reqPath.Contain("..\\") ||
       _reqPath.Contain("./") || _reqPath.Contain(".\\"))) {
  ...
  if (m_staticFileCache.find(_reqPath) != m_staticFileCache.end()) {
    ...
    if (m_staticCacheTimeout &&
        time(nullptr) - cache.m_lastupd < m_staticCacheTimeout) {
  ...
  //拼接路径
  String strFilePath =
      m_staticPath.m_filePath +
      _reqPath.Middle(m_staticPath.m_reqPath.Length(), String::end);
  ...
  if (file.Open(File::OpenMode::READ)) {
    ...
    if (m_staticCacheTimeout &&
        time(nullptr) - m_historyMode.cache.m_lastupd < m_staticCacheTimeout) {
